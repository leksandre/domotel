...
//                ->defaultHidden(),
            TD::make('title', trans('kelnik-contact::admin.title'))
                ->render(function (SocialLink $socLink) use ($coreService) {
                    return resolve(IconComponent::class, [
                        ...
                        ' ' .
                        Link::make($socLink->title)
                            ->route($coreService->getFullRouteName('contact.social.edit'), $socLink);
                }),
            ...
            TD::make('updated_at', trans('kelnik-contact::admin.updated'))
                ...
                ->defaultHidden(),
            TD::make()
                ->render(static function (SocialLink $social) use ($coreService) {
                    $str = '<div class="admin-page-list_menu">';
                    $str .= '<div class="form-group mb-0">' .
                                \view('kelnik-core::platform.booleanState', ['state' => $social->active]) .
                            '</div>';
                    $str .= Link::make()->icon('pencil')
                        ->route($coreService->getFullRouteName('contact.social.edit'), $social);
                    $str .= Button::make()->icon('bs.trash3')
                        ->action(route(
                            $coreService->getFullRouteName('contact.social.edit'),
                            ['social' => $social->getKey(), 'method' => 'removeSocial']
                        ))
                        ->confirm(trans('kelnik-contact::admin.deleteConfirm', ['title' => $social->title]));
                    $str .= '</div>';
