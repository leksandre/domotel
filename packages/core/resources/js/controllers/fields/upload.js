...
export default class extends window.Controller {
    ...
    save() {
        ...
        let loadMediaModal = window.Bootstrap.Modal.getOrCreateInstance( dropname.querySelector(`.attachment.modal`));
    ...
    resortElement() {
        ...
        elements.forEach((value, index) => {
            ...
            const input = dropname.querySelector(`input.files-${id}`);
            items[id] = index;

            if (input) {
                dropname.appendChild(input.cloneNode());
                input.remove();
            }
        });
    ...
    addSortDataAttributes(dropname, name, file) {
    ...
    initDropZone() {
        ...
        this.dropZone = new Dropzone(this.element.querySelector('#' + this.data.get('id')), {
            url: this.data.get('chunking') ? this.prefix('/core/files') : this.prefix('/systems/files'),
            //method: 'post',
            uploadMultiple: false,
            maxFilesize: this.data.get('max-file-size'),
            maxFiles: Number(this.data.get('max-files')),
            ...
            paramName: 'files',
            chunking: !!this.data.get('chunking'),
            chunkSize: Number(this.data.get('chunk-size')),
            forceChunking: !!this.data.get('chunking'),
            ...
            init: function () {
                ...
                this.on('addedfile', (e) => {
                    ...
                    editButton.addEventListener('click', () => {
                        ...
                        const attachmentModal = window.Bootstrap.Modal.getOrCreateInstance(dropname.querySelector(`.attachment.modal`));
                ...
                if (images) {
                    Object.values(images).forEach((item) => {
                        ...
                        self.addSortDataAttributes(dropname, name, item);
                    });
                }
            ...
            success(file, response) {
                ...
                self.addSortDataAttributes(dropname, name, file.data);
                self.resortElement();
            }
        });
    }
    ...
    loadMedia() {
        ...
        let loadMediaModal = window.Bootstrap.Modal.getOrCreateInstance( dropname.querySelector(`.media.modal`));
        loadMediaModal.show();
    ...
    addFile(event) {
        ...
        if (this.data.get('close-on-add')) {
            let loadMediaModal = window.Bootstrap.Modal.getOrCreateInstance( this.dropname.querySelector(`.media.modal`));
            loadMediaModal.hide();
        }
    }
    ...
    addedExistFile(attachment) {
        ...
        this.addSortDataAttributes(this.dropname, this.data.get('name'), file);
        this.resortElement();
    }
}
