...
final class FormLayout extends Rows
{
    protected function fields(): array
    {
        ...
        return [
            Title::make('')->value(trans('kelnik-estate-visual::admin.components.selectorFrame.data.form.header')),
            Input::make('data.form.text')
                ->title('kelnik-estate-visual::admin.components.selectorFrame.data.form.text')
                ->max(DataProvider::FORM_TEXT_MAX_LENGTH),
            Select::make('data.form.id')
                ->title('kelnik-estate-visual::admin.components.selectorFrame.data.form.element')
                ->options($formPlatformService->getList())
                ->empty(
                    trans('kelnik-estate-visual::admin.components.selectorFrame.data.form.noValue'),
                    DataProvider::NO_VALUE
                )
                ->help('kelnik-estate-visual::admin.components.selectorFrame.data.form.help'),
