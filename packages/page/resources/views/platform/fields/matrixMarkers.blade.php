@component($typeForm, get_defined_vars())
    <table class="matrix table table-bordered border-right-0"
        ...
        @foreach($value as $key => $row)
            @include('kelnik-page::platform.partials.fields.matrixMarkerRow', ['row' => $row, 'key' => $key])
        @endforeach
        ...
        <template class="matrix-template">
            @include('kelnik-page::platform.partials.fields.matrixMarkerRow', ['row' => [], 'key' => '{index}'])
        </template>
        </tbody>
    </table>
@endcomponent
